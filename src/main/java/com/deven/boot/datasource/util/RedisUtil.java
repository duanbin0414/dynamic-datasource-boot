package com.deven.boot.datasource.util;
...
public class RedisUtil {
     ...
     * 将字符串值 value 关联到 key 。
     * 不会更新原有key值的TTL
     * 有缺陷，value值的长度小于库中长度时会出现问题
     */
    public void setRangeString(final String key, final String value) {
        new Executor<Long>(shardedJedisPool) {

            @Override
            Long execute() {
                return jedis.setrange(key, 0L, value);
            }
        }.getResult();
    }
    ...
    /**
     * @Title: hashGetAll
     * @Description: 指定库的hashgetall
     * @param key
     * @param dbIndex 库index
    ...
    public void batchListPushHead(final String key, final List<String> values, final boolean delOld) {
        new Executor<Object>(shardedJedisPool) {
            ...
            Object execute() {
                if (delOld) {
                    ...
                    jedis.lpush(key, String.valueOf(values));
                }
                return null;
            }
        }.getResult();
    ...
    public Long zRevGetRank(final String key, final String member, final Integer ttl) {
        return new Executor<Long>(shardedJedisPool) {
            @Override
            Long execute() {
                ShardedJedisPipeline pipelined = jedis.pipelined();
     ...
     * @return
     */
    public Long zRemMemberFromSortedSet(final String key, List<String> members) {
        if (CollectionUtils.isEmpty(members)) {
            return null;
        }
        final String[] member = members.toArray(new String[members.size()]);
        return new Executor<Long>(shardedJedisPool) {
            @Override
            Long execute() {
                ShardedJedisPipeline pipelined = jedis.pipelined();
     ...
     * @return
     */
    public Double zScore(final String key, final String member, final Integer ttl) {
...
//                return jedis.zrevrangeByScore(key, max, min);
     ...
     * @return 操作是否允许
     */
    public Boolean isActionAllowed(final String actionKey, final int period, final int maxCount) {
     ...
     * @param expire    锁有效期（单位：秒）
     * @return 获取锁结果: true-成功，false-失败
     * @Title getActionLock
     * @Description 获取锁
     */
    public Boolean getActionLock(final String actionKey, final Long timeout, final int expire) {
        return new Executor<Boolean>(shardedJedisPool) {
            @Override
            Boolean execute() {
                RedisLock redisLock = new RedisLock(actionKey, jedis);
    ...
    /**
     * @param actionKey 操作key
     * @return 释放锁结果: true-成功
     * @Title freeActionLock
     * @Description 释放锁
     */
    public Boolean freeActionLock(final String actionKey) {
        return new Executor<Boolean>(shardedJedisPool) {
            @Override
            Boolean execute() {
                RedisLock redisLock = new RedisLock(actionKey, jedis);
                redisLock.unlockNoCheck();
                return true;
            }
        }.getResult();
    }
